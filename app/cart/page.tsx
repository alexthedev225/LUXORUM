...
const CartPage = () => {
  ...
  const checkoutSession = async () => {
    ...
    try {
      const body = selectedPaymentMethod
        ? { items, paymentMethodId: selectedPaymentMethod }
        : { items };
      ...
      if (data.sessionId) {
        // Paiement classique via Stripe Checkout
        ...
        if (result.error) {
          toast.error(
            result.error.message || "Erreur inconnue lors du paiement"
          );
        }
      } else if (data.paymentIntentId) {
        toast.success("Paiement réussi !");
        // Redirige vers la page succès en passant paymentIntentId en paramètre
